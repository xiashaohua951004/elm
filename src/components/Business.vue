<template>
  <div class="business_box" v-if="showMe">
      ...
      <div class="font_words">
        <div class="fw_top">
          ...
          <div class="fw_top_word clear">
            ...
            <p class="ell"><span v-if="business_info.fengniao">蜂鸟配送</span><span v-else>商家配送</span>／{{business_info.estimate_time
              }}分钟送达／配送费¥{{ business_info.send_cost }}</p>
    ...
    <!-- 商品 -->
    <section class="business_content" v-bind:style="{ height: computedContentHeight + 'px' }"
             v-show="changeShowType == 'food'">

      <ul class="business_left" ref="businessLeft">
        <li v-for="(item, index) in business_info.commodity" :key="index" @click="leftControlRightScroll(index)"
    ...
    <!-- 评价 -->
    <section class="business_rating" v-bind:style="{ height: computedContentHeight + 'px' }"
             v-show="changeShowType == 'rating'">
      <div class="total_evaluate clear">
        <div class="total_code">
          <h3>{{ business_info.shop_rating }}</h3>
          <p class="p1">综合评价</p>
          <p class="p2"><span v-if="business_info.high_or_low">高</span><span v-if="!business_info.high_or_low">低</span>于周边商家{{
            business_info.h_l_percent }}%</p>
        </div>
        <div class="detail_code">
          ...
          <p><span class="small_tit">送达时间</span><span
            style="margin-left:.1rem;">{{ business_info.estimate_time }}分钟</span></p>
        </div>
      </div>
    ...
    <section class="shopping_car" v-show="changeShowType == 'food'">
      ...
      <div :class="['car_icon', { noting: !allNub }]" ref="carIcon"
           @click="shoppingCarShow = !shoppingCarShow;alertBoxShow = false;">
      ...
      <a href="javascript:;" :class="['cartview', { cantpay: totalPrice < business_info.start_send }]"
         @click="checkout();shoppingCarShow = false">去结算</a>
    </section>
      ...
      <ul class="sp_content">
        <li v-for="(item, index) in shoppingCarList" :key="index">
          ...
          <div class="food_nub">
            ...
            <svg
              @click="add_shopping_car(item.type_accumulation, item.type_name, item.name, item.one_food_id, item.unit_price)">
              <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#cart-minus"></use>
            </svg>
          </div>
        </li>
    ...
    <div class="mask" v-if="alertBoxShow || shoppingCarShow"
         @click="alertBoxShow = false;shoppingCarShow = false">
    </div>
  </div>
</template>
<script>
